...
export class DailyTaskService {
    ...
    public registerDailyTasks(){
      ...
      scheduleJob(this.dailyTimeString, ((fireTime) => {
        ...
        this.client.channels.fetch(this.askRedditChannel)
          .then((channel) => {
            const textChannel:TextChannel = channel as TextChannel;
            this.askRedditFetcher.fetchRedditEmbed()
              .then(embed => {
          ...
          .catch(err => {
            console.error("An error occured in posting the daily reddit question");
            console.log(err);
          })
      }));
      scheduleJob(this.dailyTimeString, ((fireTime) => {
        console.log(`Starting leetcode post routine at ${fireTime}`);
        this.client.channels.fetch(this.leetcodeChannel)
          .then((channel) => {
            const textChannel:TextChannel = channel as TextChannel;
            this.leetcodeProblemSelector.selectProblem()
              .then(embed => {
                textChannel.send(embed);
              })
              .catch(err => {throw err});
          ...
          .catch(err => {
            console.error("An error occured in posting the daily leetcode question");
            console.log(err);
          })
      }));
      scheduleJob(this.updateFileTimeString, ((fireTime) => {
        console.log(`Starting leetcode problem download routine at ${fireTime}`);
        this.leetcodeProblemDownloader.downloadParsedProblems()
          .then((filePath) => {
            console.log(`Leetcode problem file located at ${filePath}`);
          })
          .catch((err) => {
            console.error("An error occured in downloading the leetcode problems");
            console.log(err);
          })
      }));
    }
